...
folder_path_R = r'C:\Users\msedo\Documents\CCEPs\Analysis data\Procedure\Asleep'
R = merge_excel_files_in_folder(folder_path_R)
R = R[(R['N1 matsumoto'] >= 0) & (R['N1 matsumoto'] <= 400) & (R['N2_Amplitude'] <= 400) & (R['N1_Latency'] <= 48)]

folder_path_AC = r'C:\Users\msedo\Documents\CCEPs\Analysis data\Procedure\Awake'
AC = merge_excel_files_in_folder(folder_path_AC)
AC = AC[(AC['N1 matsumoto'] >= 0) & (AC['N1 matsumoto'] <= 400) & (AC['N2_Amplitude'] <= 400) & (AC['N1_Latency'] <= 48)]
...
print("Statistics of Resection dataset: ")
compute_statistics(R)

print("Statistics of AC dataset: ")
compute_statistics(AC)
...
def plot_combined_violin(variable_name, svar, R, AC, units='units'):
    """
    ...
    data_R = {
        svar: ['Asleep'] * len(R[variable_name]),
    ...
    data_AC = {
        svar: ['Awake'] * len(AC[variable_name]),
        variable_name: AC[variable_name]
    }
    df_AC = pd.DataFrame(data_AC)
    
    # Combine data into a single DataFrame for plotting
    df_combined = pd.concat([df_R, df_AC], ignore_index=True)
...
plot_combined_violin('N1_Latency','Procedure', R, AC, units='ms')


# --------------------------- N1 amplitude Violin plots ---------------------------------

plot_combined_violin('N1 matsumoto','Procedure', R, AC, units='µV')

# --------------------------- P2 latency Violin plots ---------------------------------

plot_combined_violin('P2_Latency','Procedure', R, AC, units='ms')

# --------------------------- N2 latency Violin plots ---------------------------------

plot_combined_violin('N2_Latency','Procedure', R, AC, units='ms')

# --------------------------- N2 amplitude Violin plots ---------------------------------

plot_combined_violin('N2_Amplitude','Procedure', R, AC, units='µV')
