...
folder = r'C:\Users\marti\OneDrive\Documents\UPC\Quart de carrera\8th Cuatrimestre\TFG\SJD\Data Recordings\PATIENT DATA\Patient 9\SE'
...
ipd = readEDF(file=files[0])
ipd1 = readEDF(file=files[1])
...
for i in np.array(range(0, len(file_names))):
    ...
    f_signals.append(signals[i])  # Store the signal in the list
...
for idx in range(len(f_signals)):
    ...
    mask_section1 = (t >= 10) & (t <= 40)
    mask_section2 = (t >= 40) & (t <= 150)
    mask_section3 = (t >= 20) & (t <= 68)
...
plt.title('Initial and Final Signals with features')
plt.xlim([0, 250])
plt.ylim([-400, 500])
plt.legend() 
...
plt.text(80, 300, f'P1: lat.={t[max_idx_section4]:.2f} ms, amp.={max_value_section4:.2f} uV', ha='left')
plt.text(80, 260, f'N1: lat.={t[min_idx_section1]:.2f} ms, amp.={n1_matsumoto_avg:.2f} uV', ha='left')
plt.text(80, 220, f'P2: lat.={t[max_idx_section2]:.2f} ms, amp.={max_value_section2:.2f} uV', ha='left')
...
# Export DataFrame to Excel file
new_df.to_excel(r'C:\Users\marti\OneDrive\Documents\UPC\Quart de carrera\8th Cuatrimestre\TFG\SJD\Data Recordings\PATIENT DATA\Patient 9\P9_SE.xlsx', index=True)
