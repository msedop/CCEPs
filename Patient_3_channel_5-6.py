...
folder = r'C:\Users\msedo\Documents\CCEPs\SJD\Data Recordings\PATIENT DATA\Patient 4 (surgery)\Channel 5-6'
...
for i in np.array(range(0, len(file_names))):
    ...
    f_signals.append(y)  # Store the signal in the list
    ...
    # Compute z-score normalization for this signal
    normalized_y = stats.zscore(y)
    normalized_signals.append(normalized_y)
    ...
    ax2.plot(t, normalized_y, label='Normalized Signal')
...
for idx in range(len(f_signals)):
    ...
    # Coordinates of the intersection of the N1 vertical line with the P1-P2 line
...
plt.text(75, -280, f'P1: lat.={t[max_idx_section4]:.2f} ms, amp.={max_value_section4:.2f} uV', ha='left')
plt.text(75, -240, f'N1: lat.={t[min_idx_section1]:.2f} ms, amp.={n1_matsumoto_avg:.2f} uV', ha='left')
plt.text(75, -200, f'P2: lat.={t[max_idx_section2]:.2f} ms, amp.={max_value_section2:.2f} uV', ha='left')
plt.text(75, -160, f'N2: lat.={t[min_idx_section3]:.2f} ms, amp.={n2_amp_avg:.2f} uV', ha='left')
...
#df.to_excel(r'C:\Users\marti\OneDrive\Documents\UPC\Quart de carrera\8th Cuatrimestre\TFG\SJD\Data Recordings\PATIENT DATA\Patient 3 (surgery)\Channel 5-6\P3CH5-6.xlsx', index=True)
