...
folder = r'C:\Users\marti\OneDrive\Documents\UPC\Quart de carrera\8th Cuatrimestre\TFG\SJD\Data Recordings\PATIENT DATA\Patient 14\Channel 3-4'
...
for i in np.array(range(0, len(file_names))):
    ...
    for j in range(sig_num):
        y = -data[0] * 1000  # Scale the signal

        # Invert polarity for the last 4 signals
        if j >= sig_num - 4:
            y = data[0] * 1000 
...
for idx in range(len(f_signals)):
    ...
    mask_section1 = (t >= 12) & (t <= 36)
    mask_section2 = (t >= 58) & (t <= 86)
    mask_section3 = (t >= 30) & (t <= 73)
...
df.to_excel(r'C:\Users\marti\OneDrive\Documents\UPC\Quart de carrera\8th Cuatrimestre\TFG\SJD\Data Recordings\PATIENT DATA\Patient 14\Channel 3-4\P16CH3-4_ONLY.xlsx', index=True)
